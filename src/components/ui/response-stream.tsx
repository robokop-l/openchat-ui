...
function useTextStream({
  ...
  const prevTextStreamRef = useRef<string | AsyncIterable<string>>();
  ...
  // Replace the original useEffect that depends on [textStream, startStreaming]
  useEffect(() => {
    const isAppendingString = 
        modeRef.current === 'typewriter' &&
        typeof textStream === 'string' &&
        typeof prevTextStreamRef.current === 'string' &&
        textStream.startsWith(prevTextStreamRef.current) &&
        textStream.length > prevTextStreamRef.current.length;

    if (typeof textStream === "string") {
      if (modeRef.current === "fade") {
        setDisplayedText(textStream);
        updateSegments(textStream);
        // For fade mode, completion is based on whether textStream is empty or not
        // as it displays all at once after delays.
        if (textStream === "" && !completedRef.current) {
            setIsComplete(false); // Explicitly set to false if text is empty and not already marked complete
        } else if (textStream !== "") {
            // This part is tricky for fade, as completion is more about all segments faded in.
            // The original logic for fade completion is tied to onAnimationEnd of the last segment.
            // Let's ensure isComplete is true if textStream is present, actual visual completion is handled by animation.
            setIsComplete(true); 
        }
      } else { // Typewriter mode for string
        if (!isAppendingString) {
          reset(); 
        }
        
        if (textStream !== "" || (isAppendingString && textStream.length > (prevTextStreamRef.current as string)?.length)) {
            if (animationRef.current && !isAppendingString) { // If not appending and animation was running, cancel it to restart clean
                cancelAnimationFrame(animationRef.current);
                animationRef.current = null;
            }
            // Only start typewriter if there's text or if it's a meaningful append
            processStringTypewriter(textStream);
        } else if (textStream === "" && !isAppendingString) {
            // If textStream becomes empty and it's not part of an append (e.g. cleared), ensure reset happens.
            reset();
        }
      }
    } else if (textStream && typeof textStream !== "string") { // AsyncIterable
      reset();
      processAsyncIterable(textStream);
    } else { // textStream is null or undefined
      reset();
    }

    prevTextStreamRef.current = textStream;
    ...
    textStream, 
    // mode, // mode is read from modeRef.current now
    reset, 
    processStringTypewriter, 
    processAsyncIterable, 
    updateSegments,
    // setDisplayedText, // not needed as dep
    // setIsComplete // not needed as dep
  ]);
            ...
            <div className="relative">
              {segments.map((segment, idx) => {
                ...
                return (
                  ...
                  >
                    {/* It's tricky to apply ReactMarkdown to individual segments in fade mode 
                        without breaking the animation or causing re-renders.
                        For now, rendering plain text for fade mode segments. 
                        Full markdown applied once complete or if mode is typewriter.*/}
                    {segment.text}
                  </span>
                )
              })}
              {/* Render full markdown for fade when complete for better UX */}
              {isComplete && <div className="sr-only"><ReactMarkdown>{displayedText}</ReactMarkdown></div>}
            </div>
          </>
        )
